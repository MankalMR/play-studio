...
const LEVEL_1_PUZZLE: Puzzle = {
  ...
  letters: "BUDGET",
  words: [
    { word: "BUDGET", x: 0, y: 2, direction: "h" },
    { word: "DEBUG", x: 0, y: 0, direction: "v" },
    { word: "DUE", x:0, y: 0, direction: "h" },
    { word: "GET", x: 3, y: 2, direction: "v" },
  ]
};

const WordWheelGame: React.FC = () => {
  ...
  const loadLevel = useCallback(
    async (level: number) => {
      ...
      setState(prev => ({ ...prev, loading: true, message: `Building Level ${level}...` }));
      try {
        let puzzle: Puzzle;
  ...
  const handleWordSubmit = (word: string) => {
    ...
    if (targetWord) {
      if (state.foundWords.includes(word)) {
        showTemporaryMessage("ALREADY FOUND", "info");
      ...
      showTemporaryMessage("NOT IN PUZZLE", "error");
      playForeground(getPath("/media/audio/sfx/wordwheel/incorrectword.mp3"));
    }
    setCurrentGuess("");
  };
  ...
  const handleHintRequest = () => {
    ...
    if (unfound.length > 0) {
      ...
      showTemporaryMessage("HINT APPLIED", "info");
      playForeground(getPath("/media/audio/sfx/global/buttonclick.mp3"))
  ...
  const levelModal = (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/1 backdrop-blur-2xl animate-in fade-in duration-300 animate-[fadeIn_0.3s_ease-out]">
      <div className="bg-black px-[55px] lg:px-[40px] py-[50px] gap-[40px] rounded-3xl shadow-2xl flex flex-col items-center mx-[55px] rainbow-border w-full max-w-[430px] text-center">
        <h2 className="text-[48px] font-medium text-white tracking-[-1.2px] leading-[1.1]">Level {state.level} complete</h2>
  ...
  return (
    <div className="w-full h-full flex flex-col mx-auto rounded-2xl justify-center max-w-[1320px]">
      ...
      />
      <InfoDialog title="Solve the crossword" goal="Connect letters in a continuous line to form words. Guess the right words and they’ll fill in the crossword grid. Solve each word to complete the level!" onClose={() => {}} />
    </div>
  );
};
