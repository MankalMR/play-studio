    ...
    return (
        <div className="absolute inset-0 bg-black/90 z-[100] text-white pointer-events-auto backdrop-blur-md overflow-y-auto">
             <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                 <h2 className="text-3xl md:text-4xl font-black text-amber-200 mb-2 font-serif tracking-widest text-center">THE VAULT SHOP</h2>
                 <div className="flex items-center text-zinc-400 mb-6 md:mb-8">
                     <span className="text-base md:text-lg mr-2">CREDITS:</span>
                     <span className="text-xl md:text-2xl font-bold text-white">{score.toLocaleString()}</span>
                 </div>
                         ...
                         return (
                             <div key={item.id} className="bg-gray-900/80 border border-gray-700 p-4 md:p-6 rounded-xl flex flex-col items-center text-center hover:border-cyan-500 transition-colors">
                                 <div className="bg-gray-800 p-3 md:p-4 rounded-full mb-3 md:mb-4">
                                     <Icon className="w-6 h-6 md:w-8 md:h-8 text-amber-200" />
                                 </div>
                                 <h3 className="text-lg md:text-xl font-bold mb-2 font-serif">{item.name}</h3>
                                 <p className="text-zinc-500 text-xs md:text-sm mb-4 h-10 md:h-12 flex items-center justify-center italic">{item.description}</p>
                                 <button 
          ...
          <div className="absolute inset-0 flex items-center justify-center z-[100] bg-black/80 backdrop-blur-sm p-4 pointer-events-auto">
              ...
              <div className="relative w-full max-w-md rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,255,255,0.2)] border border-white/10 animate-in zoom-in-95 duration-500">
                ...
                <div className="relative w-full bg-black">
                     <img 
                      src="/mankala-runner-splash.png" 
                      alt="Mankala Runner Cover" 
                     ...
                     <div className="absolute inset-0 flex flex-col justify-end items-center p-6 pb-20 text-center z-10">
                        ...
                        >
                            <span className="relative z-10 tracking-[0.2em] flex items-center justify-center uppercase">
                                Initialize Run <Play className="ml-2 w-5 h-5 fill-black" />
                            </span>
                        </button>

                        <p className="text-zinc-500 text-[10px] md:text-xs font-bold mt-3 tracking-[0.3em] uppercase">
          ...
          <div className="absolute inset-0 bg-black/90 z-[100] text-white pointer-events-auto backdrop-blur-sm overflow-y-auto">
              <div className="flex flex-col items-center justify-center min-h-full py-8 pb-32 px-4">
                ...
                <button 
                  ...
                  className="px-10 py-4 bg-white text-black font-black text-lg md:text-xl rounded-full hover:scale-105 transition-all shadow-[0_0_20px_rgba(255,255,255,0.2)] tracking-widest"
                >
                    RUN AGAIN
                </button>
              </div>
    ...
    return (
        <div className="absolute inset-0 bg-gradient-to-b from-purple-900/90 to-black/95 z-[100] text-white pointer-events-auto backdrop-blur-md overflow-y-auto">
            <div className="flex flex-col items-center justify-center min-h-full py-8 px-4">
                ...
                <h1 className="text-3xl md:text-7xl font-black text-white mb-2 drop-shadow-[0_0_20px_rgba(255,255,255,0.2)] font-serif italic text-center leading-tight">
                    MANKALA COMPLETED
                </h1>
                <p className="text-amber-200 text-sm md:text-2xl font-serif italic mb-8 tracking-widest text-center">
                    THE LEGENDARY STRATEGY HAS BEEN MASTERED
                </p>
                ...
                <button 
                  onClick={() => { audio.init(); restartGame(); }}
                  className="px-10 py-4 bg-white text-black font-black text-lg md:text-xl rounded-full hover:scale-105 transition-all shadow-[0_0_40px_rgba(255,255,255,0.2)] tracking-widest"
                >
                    RESTART RUN
                </button>
            </div>
    ...
    <div className={containerClass}>
        ...
        <div className="flex justify-between items-start w-full">
            <div className="flex flex-col">
                <div className="text-3xl md:text-5xl font-black text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.2)] font-serif italic">
        ...
        <div className="absolute top-5 left-1/2 transform -translate-x-1/2 text-sm md:text-lg text-amber-200 font-bold tracking-[0.2em] font-serif bg-black/50 px-4 py-1 rounded-full border border-amber-500/30 backdrop-blur-sm z-50">
            LEVEL {level} <span className="text-zinc-500 text-xs md:text-sm italic">/ 3</span>
        </div>
            ...
            {MANKALA_TARGET.map((char, idx) => {
                ...
                return (
                    <div 
                        ...
                        className={`w-7 h-9 md:w-10 md:h-12 flex items-center justify-center border-2 font-black text-base md:text-xl font-serif italic rounded-lg transform transition-all duration-300`}
                    >
                        {char}
                    </div>
                );
        ...
        <div className="w-full flex justify-end items-end">
             <div className="flex items-center space-x-2 text-zinc-500 opacity-70">
                 <Zap className="w-4 h-4 md:w-6 md:h-6 animate-pulse" />
                 <span className="font-serif italic font-bold text-base md:text-xl uppercase tracking-widest">Speed {Math.round((speed / RUN_SPEED_BASE) * 100)}%</span>
             </div>
        </div>
    </div>
  );
