...
export const MANKALA_TARGET = ['M', 'A', 'N', 'K', 'A', 'L', 'A'];
const MAX_LEVEL = 3;
...
export const useStore = create<GameState>((set, get) => ({
  ...
  collectLetter: (index) => {
    ...
    if (!collectedLetters.includes(index)) {
      ...
      // LINEAR SPEED INCREASE: Add 8% of BASE speed per letter (adjusted for longer word)
      ...
      if (newLetters.length === MANKALA_TARGET.length) {
        if (level < MAX_LEVEL) {
